...
export default async function ProgressPage() {
  ...
  return (
    <section>
      ...
      <form action={addWeightAction} className="mt-5 space-y-3 rounded-[2rem] bg-white/80 p-5 shadow-sm ring-1 ring-black/5"><h2 className="font-bold">Peso corporal</h2><input name="weight" type="number" step="0.1" min="1" placeholder="Peso (kg)" required className="w-full rounded-2xl bg-[#f8faf7] px-4 py-3" /><input name="notes" placeholder="Nota opcional" className="w-full rounded-2xl bg-[#f8faf7] px-4 py-3" /><button className="w-full rounded-2xl bg-[#23352b] py-3 font-bold text-white">Añadir peso</button></form>
    </section>
  );
}
