...
export type WorkoutStep = { name: string; mode: "time" | "reps"; value: number };

export function WorkoutRunner({ name, category, minutes, steps }: { name: string; category: string; minutes: number; steps: WorkoutStep[] }) {
  const safeSteps = steps.length ? steps : [{ name: "Entrenamiento libre", mode: "time" as const, value: minutes * 60 }];
